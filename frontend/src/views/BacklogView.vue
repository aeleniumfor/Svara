<template>
  <div class="view">
    <div class="view-header">
      <h2>📋 Backlog</h2>
      <p class="view-description">やると決めたタスクのストック（今日ではない）</p>
      <p class="view-next-action">→ 編集で「Today ランク」を設定すると Today へ</p>
    </div>
    <div v-if="loading" class="loading">読み込み中...</div>
    <div v-else-if="error" class="error">{{ error }}</div>
    <div v-else-if="tasks.length === 0" class="empty">
      <p>Backlog にはまだタスクがありません</p>
      <p class="empty-hint">
        Inbox のタスクを「→next」で昇格すると、ここに表示されます
      </p>
    </div>
    <div v-else>
      <TaskCard
